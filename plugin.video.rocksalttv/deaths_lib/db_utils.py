    ...
    def import_into_db(self, full_path):
        ...
        try:
            ...
            if self.progress:
                ...
                progress.create('Death Streams', line2='Import from %s' % (full_path), line3='Importing 0 of %s' % (num_lines))
    ...
    def init_database(self, db_version):
        try:
            ...
            if db_version is not None and cur_version != db_version:
                ...
                self.progress.create('Death Streams', line1='Migrating from %s to %s' % (db_version, cur_version), line2='Saving current data.')
                self.progress.update(0)
                self.__prep_for_reinit()
    
            logger.log('Building Death Streams Database', log_utils.LOGDEBUG)
