...
class Scraper(scraper.Scraper):
    ...
    def __init__(self, timeout=scraper.DEFAULT_TIMEOUT):
        ...
        self.api = kodi.get_setting('%s-api' % (self.get_name()))
    ...
    def get_settings(cls):
        ...
        name = cls.get_name()
        settings.append('         <setting id="%s-api" type="text" label="     %s" option="hidden" default="" visible="eq(-4,true)"/>' % (name, i18n('api')))
        return settings
    ...
    def __translate_search(self, url, search_type):
        ...
        params = {'query': query, 'count': 10, 'from': 0, 'getmeta': 0}
        if self.api:
            params.update({'api': self.api})
        else:
            url = ''
        return url, params
